...
text="Andie is rejected the the the the by Blane’s friends people  people people due to her lack of social skills\
    and inappropriate clothing choices; and ultimately they are jealous \
        of her beauty and her unique style. Blane doesn’t fit in with Andie’s\
            “crowd” mostly because Duckie is jealous of him and tries to continually \
                foil him; and he doesn’t understand her friends in comparison to his own\
                    social circle. Andie on the other hand is ashamed of her home, car \
                        and her family and friends, so she tries to hide her “real life”"
...
def funcword_freq(text):
    ...
    return norm
len_feature_funcwordfreq=len(funcword_freq(text))
...
def pos_freq(text):
    ...
    return norm
len_feature_posfreq=len(pos_freq(text))
...
def punc_freq(text):
    ...
    for i in punc_freq:
        ...
        norm.append(normvals)
    return norm
len_feature_puncfreq=len(punc_freq(text))    
...
# evaluate=evaluate(test_label,classify)
# The main program
# import numpy as np
...
import pandas as pd
df_p_test=pd.DataFrame(names,values_P_test)
df_f1_test=pd.DataFrame(names, values_f1_test)
df_R_test=pd.DataFrame(names, values_R_test)
df_mean_f1_val=pd.DataFrame(names, values_mean_f1)
df_mean_recall_val=pd.DataFrame(names, values_mean_R)
df_mean_precision_val=pd.DataFrame(names, values_mean_P)
values_features=[len_feature_wordvec, len_feature_funcwordfreq, len_feature_puncfreq, len_feature_typetoken, len_feature_posfreq]
names_features=['BOW', 'function words', 'punctuation', 'typetoken', 'pos tags']


no_feature_count=pd.DataFrame(values_features, names_features)
